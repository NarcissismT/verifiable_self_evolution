#!/usr/bin/env python3
"""Trusted execution boundary for an untrusted SDK candidate."""
...
def load_candidate(path: Path, sdk_root: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("vse_sdk_candidate", path.resolve())
...
def initialize_runtime_seed(seed: int) -> None:
    random.seed(int(seed))
    try:
        import numpy as np
        np.random.seed(int(seed))
    except ImportError:
        # The frozen container includes the pinned NumPy version.  Keeping the
        # host-side wrapper usable without NumPy lets contract tests remain
        # dependency-light; candidates that import NumPy still fail closed.
        return
...
def main() -> int:
    ...
    try:
        ...
        sealed = {"ok": True, "result": validate_result(result, int(problem["dimension"]), budget),
                  "error_category": "", "error_message": ""}
    except Exception as error:
        sealed = {"ok": False, "result": {}, "error_category": type(error).__name__,
