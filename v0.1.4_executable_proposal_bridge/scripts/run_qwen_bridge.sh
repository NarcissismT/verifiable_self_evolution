...
docker run --rm --gpus device=0 --network none \
  ...
  -v "$ROOT/cases:/cases:ro" \
  -v "$OUTPUT_ROOT/proposals:/output:rw" \
  -v "$(cd "$ROOT/.." && pwd):/repo:ro" \
  "$IMAGE" python /repo/v0.1.4_executable_proposal_bridge/scripts/generate_bridge_proposals.py \
  --model-path /model --model-digest "$MODEL_DIGEST" \
  --case-manifest /cases/case_manifest.json --output-root /output \
  --max-new-tokens 3072
GENERATION_STATUS=$?
set -e
...
docker run --rm --network none \
  ...
  -v "$(cd "$ROOT/.." && pwd):/repo:ro" \
  -v "$ROOT:/bridge:ro" \
  -v "$OUTPUT_ROOT/proposals:/proposals:ro" \
  -v "$OUTPUT_ROOT/run:/run:rw" \
  --entrypoint python "$IMAGE" /bridge/scripts/run_bridge_gate.py \
  --case-manifest /bridge/cases/case_manifest.json \
  --proposals-root /proposals --output-root /run \
  --model-digest "$MODEL_DIGEST" --execution-container-digest "$IMAGE_DIGEST"
GATE_STATUS=$?
set -e
...
value = {
    ...
    "gate_completed": (root / "run" / "bridge_report.json").is_file(),
}
(root / "run" / "overall_report.json").write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
print(json.dumps(value, indent=2, sort_keys=True))
PY
echo "v0.1.4 bridge completed: $OUTPUT_ROOT/run (generation=$GENERATION_STATUS gate=$GATE_STATUS)"
